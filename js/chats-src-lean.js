// https://unpkg.com/browse/leancloud-storage@4.13.4/dist/
// https://docs.leancloud.cn/sdk/storage/guide/js/
AV.init({
  appId: "p8OlC6lYButDR6STtIVJiAnt-gzGzoHsz",
  appKey: "QyAxF2QdQa2EOt5jlMlHtkVv",
  serverURL: "https://p8olc6ly.lc-cn-n1-shared.com",
});
...
function queryChats(skip, size) {
    ...
    query.find().then(res => {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            var html = "<b>" + res[i].attributes.name + "</b>";
            // https://www.gravatar.com/avatar/
            // https://cn.gravatar.com/avatar/
            // https://cdn.v2ex.com/gravatar/
            // https://dn-qiniu-avatar.qbox.me/avatar/
            // 统一换成七牛的的转发
            if (res[i].attributes.web) {
                if (res[i].attributes.web.indexOf("http") != -1) {
                    html = "<a href='" + res[i].attributes.web + "' target='_blank'><img src='https://dn-qiniu-avatar.qbox.me/avatar/" + res[i].attributes.email.MD5(32) + "' /></a>" + html;
                } else {
                    html = "<a href='http://" + res[i].attributes.web + "' target='_blank'><img src='https://dn-qiniu-avatar.qbox.me/avatar/" + res[i].attributes.email.MD5(32) + "' /></a>" + html;
                }
            } else {
                html = "<img src='https://dn-qiniu-avatar.qbox.me/avatar/" + res[i].attributes.email.MD5(32) + "' />" + html;
            }
            /* if (res[i].attributes.web !== '') {
                html = html + "<label>" + res[i].attributes.web + "</label>";
            } else {
                if (res[i].attributes.email !== '') {
                    html = html + "<label>" + res[i].attributes.email + "</label>";
                }
            } */
            html = html + "<label>" + res[i].attributes.date + "</label><p>" + res[i].attributes.msg + "</p>";
            li.innerHTML = html;
            ul.appendChild(li);
        }
        // 留言为空
...
function addChats(btn) {
    ...
    const query = new AV.Object("Chat");
    query.set("url", url);
    query.set("name", name);
...
var myHome = new Vue({
    ...
    methods: {
        queryData() {
            ...
            query.descending("date");
            query.limit(100);
            query.select(["msg", "-objectId", "-createdAt", "-updatedAt"]);
            query.find().then(res => {
                // console.log(res);
                var msgArray = []
                for (var key in res) {
                    msgArray.push(res[key].attributes.msg)
                }
                // 判断手机还是PC
